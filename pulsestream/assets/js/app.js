(() => {
  ...
  function normalizeUserUrl(value) {
    ...
    // Remove wrappers often included by share sheets, e.g. <https://...>
    v = v.replace(/^<+|>+$/g, '');

    // Collapse all whitespace/newlines inside pasted values.
    v = v.replace(/\s+/g, '');

    // Remove trailing punctuation copied from chat messages.
    v = v.replace(/[.,;!?]+$/g, '');

    // Accept host-only links by adding https:// automatically.
    if (/^[a-z0-9.-]+\.[a-z]{2,}(?:\/|$)/i.test(v) && !/^https?:\/\//i.test(v)) {
      v = 'https://' + v;
    }
  ...
  async function fetchInfo() {
    ...
    const infoEndpoint = apiEndpoint('api/info.php');
    clearAlert();
    if (!url) {
    ...
    try {
      ...
      const msg = (err && err.message) ? String(err.message) : 'Unknown extraction error.';
      if (/expected pattern/i.test(msg)) {
        showAlert(`Request failed: ${msg} (endpoint: ${infoEndpoint})`);
      } else {
        showAlert(msg);
      }
      console.error('[pulsestream] fetchInfo failed', { msg, infoEndpoint, url, err });
      setProgress(false);
    } finally {
      state.loading = false;
